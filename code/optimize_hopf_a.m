%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% HOPF MODEL OPTIMISATION - bifurcation parameter a as control parameter
%
% Written by 
% Gustavo Deco
% Edited by
...
% Ludovica Romanin Novemeber 2020 - simulation optimized on parameter a
%
...
% OUTPUTs:         
%
% - a_coeff: bifurcation parameter (a)
% - PTRsimul: 
...
dsig = sqrt(dt)*sig; % to avoid sqrt(dt) at each time step
                  ...
                  %%%%%%%%%%%% B: SIMULATION %%%%%%%%%%%%
%%
ia=1; % count for the global wieght iteration
we = 1; %% Global Coupling Factor G: fixed, complete synchronization

a_coeff = [-0.5:0.02:0.5]; % test different bifurcation parameters (as in Jobst et al., 2017)
a=ones(N,2); % bifurcation parameter vector 

Na = length(a_coeff);
PTRsimul=zeros(Na,NumClusters,NumClusters);
Pstatessimul=zeros(Na,NumClusters);

FCsim = zeros(Na, N, N);

for a_i=a_coeff % loops over bifurcation parameter a
    %% B2: FINAL SIMULATION
    ...
    metastability(ia)=abs(metastabilitydata-std(sync));
    ...
    [H,P,ksdist(ia)]=kstest2(phfcd,phfcddata);
    ...
    klpstates(ia)=0.5*(sum(Pstates.*log(Pstates./P2emp))+sum(P2emp.*log(P2emp./Pstates)));
    ...
    kldist_pms(ia)=KLdist(PTR2emp,PTRsim); % PTR2emp, parameter from empiricalLEiDA.mat
    entropydist_pms(ia)=EntropyMarkov(PTR2emp,PTRsim); 
    
    PTRsimul(ia,:,:)=PTRsim;
    %%%
    Pstatessimul(ia,:)=Pstates;
...
% when saving the data specific K_ (number of clusters), and atlas used
save ../results/hopf_model/optimizedhopf_K_a.mat a_coeff PTRsimul Pstatessimul metastability ksdist klpstates kldist_pms entropydist_pms fitt NSUB f_diff FCsim FCemp;

save('../results/hopf_model/HopfModel_results_K_a.mat', '-v7.3');
