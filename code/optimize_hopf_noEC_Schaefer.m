...
%% Load LEiDA results
load  empiricalLEiDA_K_16_N800.mat; % file with results computed in LEiDA_2Conditions.m
...
%load sc_dbs80.mat;
load meanSC_100ERG_Schaefer800.mat; %structural with Schaefer
C=meanSC;
C=C/max(max(C))*0.2;
%C=C/max(max(C));

% remove the areas with timecourses at zero from the SC matrix
load areas_zero_800.mat;
C(areas_zero,:) = [];
C(:,areas_zero) = [];
...
%% Saving
save optimizedhopf_N800_K16.mat WE PTRsimul Pstatessimul metastability ksdist klpstates kldist_pms entropydist_pms fitt NSUB f_diff FCsim FCemp;

%save('fcd_result_N800_K16.mat', 'phfcd_sim', '-v7.3');

save HopfModel_results_N800_K16.mat;
