...
function [sig_meas_unc,sig_meas_corr]=permutation_onesample_ttest_2021(data_group,data_value,alpha,nSurr,mode,c)
...
for i=1:nMeas
    ...
    for j=1:nSurr
        r=randperm(n,1);%random number between 1 and sample size n, to be assigned to value
        remaining_indexes=setdiff(1:n+1,r);%remaining indexes go in the group mean
        data_perm(:,1)=[data_group(r);data(remaining_indexes,i)];
        
        switch mode
            case 'mean'%1)consider mean
                diff_surr(i,j)=mean(data_perm(2:n,1))-data_perm(1,1);
            case 'central_mean' %2)consider mean of central values
                sdata_perm1(:,1)=sort(data_perm(2:n,1),'ascend');
                diff_surr(i,j)=mean(sdata_perm1(m1-3:m1+3,1))-data_perm(1,1);
            case 'median' %3)consider median
                diff_surr(i,j)=median(data_perm(2:n,1))-data_perm(1,1);
        end
...
for i=1:nMeas
    if c=='3'
        ...
        %%%%Giulia modifiche 12.01.2021
        thr(i,1)=s((alpha*(nSurr+1))+1);
        if abs(diff_real(i,1))>=thr(i,1)
            if diff_real(i,1)>0
                sig_meas_unc(1,i)=1;
            elseif diff_real(i,1)<0
                sig_meas_unc(1,i)=-1;
            end
        end
        %%%%
