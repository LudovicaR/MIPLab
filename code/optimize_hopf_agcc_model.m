%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% HOPF MODEL OPTIMISATION - AgCC model 
%
% Written by 
% Gustavo Deco
% Edited by
...
% Ludovica Romanin December 2020 - AgCC model
%
...
%
% Model simulation based on the fitting of a model to the AgCC dataset.
% We select only the complete AgCC to have lower inter-subjects variations.
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%% Load LEiDA results
load ../results/leida/LEiDA_results_K_14_BN.mat; % file with results from LEiDA

% index of subjects with complete AgCC
idx_complete = [3,4,5,6,7,8,9,13];

% parameters from empiricalLEiDA.mat
P1emp=mean(P1emp(idx_complete,:)); % mean probability of occurrence of the complete AgCC group 
...
load ../data/meanSCs/meanSC_BN_AgCC.mat; %structural with BN246
C=meanSC;
C=C/max(max(C))*0.2;
...
load ../data/TCS/AgCC_TCS_246BN.mat;
% remove the areas with timecourses at zero 
AgCC_TCS(areas_zero,:,:) = [];

% select only the complete AgCC for the model 
AgCC_TCS = AgCC_TCS(:,:,idx_complete);

TP=size(AgCC_TCS,2);
NSUB=size(AgCC_TCS,3);
...
FCsim = zeros(NWE, N, N); % simulated FC matrix
...
for we=WE % loops over changing coupling constant G
    ...
    klpstates(iwe)=0.5*(sum(Pstates.*log(Pstates./P1emp))+sum(P1emp.*log(P1emp./Pstates)));
    ...
    kldist_pms(iwe)=KLdist(PTR1emp,PTRsim); % PTR1emp, parameter from empiricalLEiDA.mat
    entropydist_pms(iwe)=EntropyMarkov(PTR1emp,PTRsim); 
...
%% Saving
save ../results/hopf_model/optimizedhopf_K_14_BN_agcc.mat WE PTRsimul Pstatessimul metastability ksdist klpstates kldist_pms entropydist_pms fitt NSUB f_diff FCsim FCemp;

save('../results/hopf_model/HopfModel_results_K_14_BN_agcc.mat', '-v7.3');
