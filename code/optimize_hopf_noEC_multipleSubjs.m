...
%% Load LEiDA results
load  empiricalLEiDA_K_10_15.mat; % file with results computed in LEiDA_2Conditions.m
...
%load sc_dbs80.mat;
load meanSC_56HC_Desikan.mat; %TODO change for structural with Schaefer!!
C=meanSC;
%C=C/max(max(C))*0.2;
C=C/max(max(C));
...
for s=1:n_sub_tot
    ...
    irand = randi([1 28],1,10); % to get 10 random subject data
    ...
    for nsub=1:NSUB
        signaldata=Controls_TCS_rand(:,:,nsub);
...
%% Saving
save optimizedhopf_K15_sub.mat WE PTRsimul_sub Pstatessimul_sub metastability_sub ksdist_sub klpstates_sub kldist_pms_sub entropydist_pms_sub fitt_sub f_diff_sub FCsim_sub FCemp_sub omega_sub BOLD_sub;

%save('fcd_result_K15_sub.mat', 'phfcd_sim', '-v7.3');

save HopfModel_results_K15_sub.mat;
